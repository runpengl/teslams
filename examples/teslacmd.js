...
const argv = require("optimist")
  ...
  .describe(
    "X",
    "Check if car is plugged in and continue only if connected to a charger"
  )
  .alias("Z", "isawake")
  ...
  .describe(
    "O",
    "Select the vehicle offset for accounts with multiple vehicles"
  )
  .default("O", 0)
  .alias("R", "range")
  ...
  .describe(
    "S",
    'Move the car sunroof to: "close", "vent", "comfort", "open" or any percent'
  )
  .alias("T", "temp")
...
function parseArgs(vehicle) {
  ...
  if (argv.c) {
    teslams.get_charge_state(vid, cs => {
      if (argv.metric) {
        if (cs.battery_range !== undefined)
          cs.metric_battery_range = (cs.battery_range * 1.609344).toFixed(2);
        if (cs.est_battery_range !== undefined)
          cs.metric_est_battery_range = (
            cs.est_battery_range * 1.609344
          ).toFixed(2);
        if (cs.ideal_battery_range !== undefined)
          cs.metric_ideal_battery_range = (
            cs.ideal_battery_range * 1.609344
          ).toFixed(2);
      }
      pr(cs);
    });
  ...
  if (argv.d) {
    teslams.get_drive_state(vid, ds => {
  ...
  if (argv.roof !== undefined) {
    if (argv.roof >= 0 && argv.roof <= 100) {
      ...
      err = new Error(
        "Invalid sun roof state. Specify 0-100 percent, 'open', 'close', 'comfort' or 'vent'"
      );
      return pr(err);
    }
  }
  ...
  if (argv.temp !== undefined) {
    if (argv.temp <= teslams.TEMP_HI && argv.temp >= teslams.TEMP_LO) {
      ...
      err = new Error(
        `Invalid temperature. Valid range is ${teslams.TEMP_LO} - ${teslams.TEMP_HI} C`
      );
      return pr(err);
    }
  }
...
teslams.all(
  { email: creds.username, password: creds.password, token: creds.token },
  (error, response, body) => {
    let data;
    let vehicle;
    ...
    if (argv.isawake && vehicle.state == "asleep") {
      ...
      teslams.get_charge_state(vehicle.id, ({ charging_state }) => {
